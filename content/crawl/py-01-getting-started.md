...
The Python client is a small library that sends your requests to a running AstraeaDB server. The server is the database program running in the background, and the client is the code in your program that talks to it. On your own computer they communicate at the address `127.0.0.1:7687`, where `127.0.0.1` means this machine and `7687` is the port, a numbered channel the server listens on. The first step is to get the server running. Because AstraeaDB is written in Rust, you need Rust's build tools, which you can get from [rustup.rs](https://rustup.rs) if you do not already have them.

<!-- include: _shared/install-server.md -->

Open a second terminal window for your Python work. A few other commands are worth knowing:
...
The `with` form above is the safest way to run a single piece of work, because it always closes the connection. For the rest of this lesson you will build up a graph across several steps, so it is easier to open one connection with `connect()` and keep it open until the end. You will close it yourself in the last step.
...
The remaining examples reuse the `client` you opened above, so run them in the same session, one after another. Start by reading a single node. The `get_node(id)` method returns a dictionary containing the node's `labels` and `properties`:
...
Because you opened this connection with `connect()` rather than a `with` block,
you close it yourself when you are finished:

```python
client.close()
```

The server keeps running and your graph stays on disk, so the next post can pick
